...
IStrategyComponent* ProxyStrategyFactory(ISimulatorFacade* simFacade)
{
	return new UE4StrategyProxy(simFacade);
};
...
void FusionCrowdLinkUE4::StartFusionCrowd(char* navMeshDir)
{
	...
	strategyId = builder->WithExternalStrategy(ProxyStrategyFactory);
