...
namespace FusionCrowd
{
	namespace AgentShape
	{
		...
		float Ellipse::DistanceOfClosestApproach(const Ellipse& other) const
		{
			...
			Vector2 c1c2;
			(other._center - _center).Normalize(c1c2);
		...
		float Ellipse::ApproximateMinimumDistance(const Vector2& pt) const
		{
			...
			float dist = pSpace.Length();
			Vector2 dir = pSpace / dist;
			Vector2 boundPoint(_majorAxis * dir.x, _minorAxis * dir.y);
			float radius = boundPoint.Length();
			// Note: if the point lies INSIDE the ellipse, this value will be
			//		negative.
			return dist - radius;
		}

		float Ellipse::RadiusInPointDirection(const DirectX::SimpleMath::Vector2& pt) const
		{
			Vector2 p = ToEllipseSpace(pt);
			Vector2 dir;
			p.Normalize(p);
			Vector2 boundPoint(_majorAxis * dir.x, _minorAxis * dir.y);
			return boundPoint.Length();
		}

		float Ellipse::RadiusInDirection(const DirectX::SimpleMath::Vector2& dir) const
		{
		...
		DirectX::SimpleMath::Vector2 Ellipse::ToEllipseSpace(const DirectX::SimpleMath::Vector2& pt) const
		{
		...
		DirectX::SimpleMath::Vector2 Ellipse::FromEllipseSpace(const DirectX::SimpleMath::Vector2& pt) const
		{
