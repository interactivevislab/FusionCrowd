...
#define _Use_decl_annotations_
	...
	namespace GpuHelper
	{
		...
		HRESULT CreateStructuredBuffer(_In_ ID3D11Device* pDevice, _In_ UINT uElementSize, _In_ UINT uCount,
			_In_reads_(uElementSize*uCount) void* pInitData, _In_ D3D11_CPU_ACCESS_FLAG cpuAccessFlag, _Outptr_ ID3D11Buffer** ppBufOut);
		...
		void WriteDataToBuffer(_In_ ID3D11Buffer* buffer, _In_reads_(dataSize)void* dataSource, _In_ size_t dataSize, _In_ ID3D11DeviceContext* context);
		void ReadDataFromBuffer(_In_ ID3D11Buffer* buffer, _In_ void* dataDest, _In_ size_t dataSize, _In_ ID3D11DeviceContext* context);
