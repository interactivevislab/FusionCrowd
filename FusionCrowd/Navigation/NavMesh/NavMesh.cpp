...
#pragma endregion
	...
	//cut polygon functions
	float NavMesh::CutPolygonFromMesh(FCArray<NavMeshVetrex> & polygon) {
		...
		return DivideNode(&nodes[0], v0, v1);
	}

	float NavMesh::DivideNode(NavMeshNode* node, DirectX::SimpleMath::Vector2* v0, DirectX::SimpleMath::Vector2* v1) {
		...
		for (int i = 0; i < poly->vertCount; i++) {
			...
			//is on segment?
			float d = 1.05;
			if (((xev0 >= tmp0.x && xev0 <= tmp1.x) || (xev0 <= tmp0.x && xev0 >= tmp1.x)) &&
