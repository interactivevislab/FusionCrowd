...
namespace FusionCrowd
{
	namespace Zanlungo
	{
		...
		void ZanlungoComponent::ComputeNewVelocity(FusionCrowd::Agent* agent)
		{
			...
			Vector2 force((agent->_velPref.getPreferredVel() - agent->_vel) * _agents[agent->_id]._mass / TAU);
			...
			if (interacts) {
				...
				// obstacles
				Vector2 futurePos = agent->_pos + agent->_vel * T_i;
...
#define COLLIDE_PRIORITY
			...
			for (size_t j = 0; j < agent->_nearAgents.size(); ++j) {
				...
				Vector2 relPos = agent->_pos - other->_pos;
...
#define PRECISE
	...
	// OPPOSITE directions for convergence
		...
		float ZanlungoComponent::RightOfWayVel(FusionCrowd::Agent* agent, Vector2& otherVel, const Vector2& otherPrefVel,
		...
		Vector2 ZanlungoComponent::AgentForce(FusionCrowd::Agent* agent, const Agent* other, float T_i) const
		{
			...
			Vector2 futPos = agent->_pos + myVel * T_i;
			...
			if (weight > 1.f) {
				...
				if (prefSpeed < 0.0001f) {
					...
					Vector2 currRelPos = agent->_pos - other->_pos;
			...
			float magnitude = weight * _agentScale * (agent->_vel - other->_vel).Length() / T_i;
