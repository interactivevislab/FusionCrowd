#include "../../Agents/Events/EventEffectAgentState.h"
...
namespace Menge {
	...
	State * EventEffectAgentState::StateForAgent() const {
		...
		if (!state) throw EventFatalException("Trying to switch to an unknown state: " +
											  stateName);
		return state;
	}
	...
	//			"from specification line " << selectorNode->Row() << ".";
	//		return false;
	//	}
