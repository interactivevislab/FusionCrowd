...
namespace GCF {
	...
	public:
		 ...
		 */
		inline void setOrientation( float angle ) { _cosPhi = cos(angle); _sinPhi = sin(angle); }
		 ...
		 */
		inline void setAxes( float major, float minor ) { _majorAxis = major; _minorAxis = minor; }
	...
	protected:
		 ...
		 *				Oriented with angle phi.  This is cosine
		 *				of that angle.
		 */
		float		_cosPhi;
		...
		/*!
		 *	@brief		Part of the orientation of the ellipse.
		 *				Oriented with angle phi.  This is sine
		 *				of that angle.
		 */
		float		_sinPhi;
		 ...
		 *				The semi-major axis is the "direction" the
		 *				ellipse is oriented.  So, when phi is zero,
		 *				the semi-major axis is aligned with the
		 *				x-axis.
		 *				It need not be a bigger value than the semi-minor
		 *				axis.
		 */
		float		_majorAxis;
		 ...
		 *				The semi-minor axis is perpendicular to the "direction" the
		 *				ellipse is oriented.  So, when phi is zero,
		 *				the semi-minor axis is aligned with the
		 *				y-axis.
		 *				It need not be a smaller value than the semi-major
		 *				axis.
		 */
		float		_minorAxis;
	};
