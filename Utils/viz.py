...
AgentInfo = namedtuple("AgentInfo", ["agent_id", "pos", "orientx", "orienty", "R"])
...
def redraw_positions(canvas: Player, tr, frame, ovals=None):
    if ovals is None:
        ovals = dict()
...
if __name__ == "__main__":
    ...
    ovals = redraw_positions(canvas, tr=tr, frame=0)

    canvas.set_redraw(lambda frame: redraw_positions(canvas, tr=tr, frame=int(frame), ovals=ovals))
