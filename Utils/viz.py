...
AgentInfo = namedtuple("AgentInfo", ["agent_id", "pos", "orient", "R", "server_id"])
...
def read_trajectories(filename):
    ...
    with open(filename) as csvfile:
        ...
        for step, row in enumerate(traj_file):
            ...
            for agent_id, x, y, orientx, orienty, radius, server_id in grouper(row[1:], 7):
                info = AgentInfo(int(agent_id), (float(x), float(y)), (float(orientx), float(orienty)), float(radius), int(server_id))
...
def draw_trajectories(canvas: Player, tr):
    for id, agent_infos in tr.pos.items():
        for a1, a2 in pairwise(agent_infos.values()):
            color = all_colors[a1.server_id % len(all_colors)]
            canvas.line(a1.pos, a2.pos, color, width=2)
...
def redraw_positions(canvas: Player, tr, frame, size=1.0, ovals=None,orint=None):
    ...
    for agent_id, infos in tr.pos.items():
        ...
        if agent_id in ovals:
            canvas.recolor_item(ovals[agent_id], all_colors[infos[frame].server_id % len(all_colors)])
            ...
            ovals[agent_id] = canvas.circle(
                ...
                all_colors[infos[frame].server_id % len(all_colors)]
            )
