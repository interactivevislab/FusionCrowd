...
Menge::BFSM::FSM * CreateAndLink::BuildFSM(Menge::BFSM::FSMDescrip & fsmDescrip, Menge::Agents::SimulatorInterface * sim, bool VERBOSE)
{
...
bool CreateAndLink::finalize(Menge::Agents::SimulatorInterface * sim, Menge::BFSM::FSM * fsm)
{
	using namespace Menge;
	using namespace BFSM;
	using namespace Agents;
...
int CreateAndLink::simMain(Menge::SimulatorDBEntry * dbEntry, const std::string & outFile)
{
	using namespace Menge;
	...
	simTest = dbEntry->getNewSimulator();
	...
	generator->setAnchor(Menge::Vector2(-8, -8));
	generator->setOffset(Menge::Vector2(-2, -2));
...
int CreateAndLink::GetAgentCount()
{
	return agentCount;
}

float* CreateAndLink::Update()
{
	bool running = true;
	float* posArray = new float[agentCount *2];
